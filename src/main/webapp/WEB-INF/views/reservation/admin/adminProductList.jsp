	...
	<div id="select">
		<form action="/reservation/admin/adminSearchProduct" method="get">
			...
			<c:if test="${pi.currentPage - 1 != 0}">
				<a href="/reservation/admin/adminProdctList?page=1" class="naviBtn">
					◀️◀️ </a>
			</c:if>
			<!--'<' 누르면 현재페이지 -1 한 페이지를 보여주기 -->
			<c:if test="${pi.currentPage - 1 != 0}">
				<a
					href="/reservation/admin/adminProdctList?page=${pi.currentPage - 1 }"
					class="naviBtn"> ◀️ </a>
			</c:if>
			<c:if test="${pi.currentPage - 1 == 0}">
			...
			<c:forEach begin="${pi.startNavi }" end="${pi.endNavi }" var="p">
				<c:url var="pageUrl" value="/reservation/admin/adminProdctList">
			...
			<c:if test="${pi.currentPage + 1 <= pi.maxPage}">
				<a href="/reservation/admin/adminProdctList?page=${pi.maxPage }"
					class="naviBtn"> ▶️▶️ </a>
			</c:if>
		</form>
	</div>
